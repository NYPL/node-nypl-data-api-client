...
class Client {
  ...
  _doHttpMethod (method, path, options) {
    ...
    let cached = null
    if (options.cache && (cached = this.cache.get(cacheKey))) return cached
    ...
    if (options.authenticate) {
      request = this._doAuthenticatedRequest(requestOptions)
        .then(handleCache)
    } else {
      request = this._doRequest(requestOptions)
        .then(handleCache)
    }

    if (options.cache) this.cache.set(cacheKey, request)
    ...
    return new Promise((resolve, reject) => {
      ...
      request(requestOptions, (error, resp, body) => {
        ...
        // Sanity check response, trigger error if empty:
          ...
          reject(new Error('Invalid response: for ' + options.uri + ': ' + JSON.stringify(body)))
